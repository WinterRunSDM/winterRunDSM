...
ggplot(shannon_di_size) + 
  ...
  labs(title = "Size Class Diversity")+
  scale_color_manual(values = colors)+
  theme_plots
```
...
We show two plots: one shows the Shannon Diversity Index calculated for size class and month of ocean entry, and one shows the proportions per year. The Shannon Diversity Index measures evenness and diversity: a higher value indicates greater evenness and diversity, while a low number may indicate domination by one group (in this case, size class and month of ocean entry combination groupings).
...
Defined as populations not meeting independent population criteria. This cannot be calculated for the first year of the time series as it requires a previous year of data to calculate growth rate and CRR.
