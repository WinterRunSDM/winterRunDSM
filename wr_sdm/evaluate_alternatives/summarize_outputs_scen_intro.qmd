...
obj_metrics <- read_excel(here("wr_sdm/documentation/objectives_metrics.xlsx"))
...
spawners_split2 <- baseline_results$spawners |> 
  as.data.frame() |> 
  mutate(watershed = winterRunDSM::watershed_labels,
         scenario = "baseline") |> 
  bind_rows(portfolio_eg_results2$spawners |> 
              as.data.frame() |> 
              mutate(watershed = winterRunDSM::watershed_labels,
...
p1 <- (spawner_plot <- ggplot(spawners_split) + 
...
p2 <- (spawner_plot <- ggplot(spawners_split2) + 
  geom_line(aes(sim_year, spawners, color = scenario)) +
  geom_point(aes(sim_year, spawners, color = scenario, shape = scenario), size = 3) +
    labs(title = "Spawners by watershed")+
    facet_wrap(~watershed, nrow = 2) + 
...
### Proportion of Above Shasta habitat occupied
```{r}
ggplot(abv_dam_habitat_used) + 
  geom_line(aes(sim_year, abv_dam_habitat_used, color = scenario)) +
  geom_point(aes(sim_year, abv_dam_habitat_used, color = scenario, shape = scenario), size = 3) +
  labs(title = "Proportion of above Shasta habitat used", y = "Proportion habitat")+
  scale_color_manual(values = colors)+
  theme_plots
```
