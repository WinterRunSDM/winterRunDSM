...
The average proportion of days within the months of May-August that are below `53.5 F` in the HEC 5Q modeled temperature time series from 1980-2000 is `r mean(us_modeled_props$prop_below_threshold)`. 
...
We used `MR4A` as the relevant gage for representing spawning habitat in the Lower McCloud River. The value by which to scale the total egg-to-fry survival is `(0.28 - 0.57) / 0.57 = -0.51` which results in a scale factor of `1 + -0.51 = 0.49`. 
...
The value by which to scale the total egg-to-fry survival is `(0.625 - 0.57) / 0.57 = 0.10` which results in a scale factor of `1 + 0.10 = 1.10`. 

### Little Upper Sacramento River (ASD-5 & ASD-6)

![](md_graphs/md_asl_map.png)

We used `11342000` as the relevant gage for representing spawning habitat in the Little Upper Sacramento River. The value by which to scale the total egg-to-fry survival is `(0.17 - 0.57) / 0.57 = -0.70` which results in a scale factor of `1 + -0.70 = 0.3`.
...
wr_sdm_egg_to_fry_values <- tibble("Station ID" = c("11342000", "MRA", "MR4A", NA_character_),
                          ...
                          "Egg-to-Fry Scale Factor" = c(0.30, 1.74, 0.49, 1.10))
