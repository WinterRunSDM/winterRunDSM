...
**Approach:** Take temperature timeseries from representative locations and calculate the proportion of days from May-October (spawning in the lifecycle model is May-October but we include through October to account for emergence) where the temperature is below `53.5 F`. Calculate how this proportion relates to modeled temperature data on the Upper Sacramento River and use that ratio to modify the component of the egg-to-fry submodel that models the effect of temperature, effectively scaling the egg-to-fry survival by the suitable temperatures for the proposed action.
...
Additionally, for below Keswick temperatures representing baseline, we use the HEC 5Q modeled average temperature time series from the BELOW CLEAR CREEK node using the Action 5 CalSim 3 run.

```{r, echo = FALSE, warning = FALSE, message = FALSE}
us_modeled_temps <- read_csv(here("data-raw", "habitat-data-docs", "hec5q_blw_clear_creek_temps.csv")) |> 
  filter(watershed == "Upper Sacramento River",
         month(date) %in% 5:10) |>
  mutate(year = year(date), 
         month = month(date), 
         day = day(date), 
         fake_date = as.Date(paste0("1979-", month, "-", day)))
```

```{r, echo = FALSE, warning = FALSE, message = FALSE}
us_modeled_temps |> 
  group_by(fake_date) |> 
  summarise(mean_temps_across_years = mean(mean_daily_temp_F, na.rm = T)) |> 
  ggplot(aes(x = fake_date, y = mean_temps_across_years)) + 
  geom_line() + 
  geom_hline(aes(yintercept = 53.5), linetype = "dashed") +
  annotate("text", x = min(us_modeled_temps$date), y = 53.8, label = "53.5°F",
           hjust = 0, size = 3, color = "gray30") +
  theme_minimal() +
  labs(x = "Simulation Month",
       y = "Mean temperature (F)",
       title = "Mean temperatures at HEC 5Q blw clear creek",
       subtitle = "Temperatures summarized by date across 1979-2000")

us_modeled_props <- us_modeled_temps |> 
  mutate(below_threshold = ifelse(mean_daily_temp_F <= 53.5, TRUE, FALSE)) |> 
  group_by(year) |> 
  summarise(prop_below_threshold = sum(below_threshold, na.rm = T) / n()) |> 
  ungroup()

# comp <- us_modeled_temps |> 
#   group_by(fake_date) |> 
#   summarise(mean_temps_across_years = mean(mean_daily_temp_F, na.rm = T)) |> 
#   ungroup() |> 
#   mutate(below_threshold = ifelse(mean_temps_across_years <= 53.5, TRUE, FALSE)) |> 
#   group_by(year(fake_date)) |> 
#   summarise(prop_below_threshold = sum(below_threshold, na.rm = T) / n()) |> 
#   ungroup()

knitr::kable(us_modeled_props)
```

The average proportion of days within the months of May-October that are below `53.5 F` in the HEC 5Q modeled temperature time series from 1980-2000 is `r mean(us_modeled_props$prop_below_threshold)`. 
...
We used `MR3A` as the relevant gage for representing the Lower McCloud River based on its proximity to existing streamside egg incubators. The value by which to scale the total egg-to-fry survival is `0.92 / 0.59 = 1.56`. 

```{r, echo = FALSE, warning = FALSE, message = FALSE}
mr3a <- temps |> 
  filter(location == "MR3A",
         statistic == "avg",
         month(date) %in% 5:10)
...
mr3a |>
  ...
  annotate("text", x = min(mr3a$date), y = 54.5, label = "53.5°F",
           hjust = 0, size = 3, color = "gray30") +
  labs(x = "Date", y = "Temperature (F)",
       title = "MR3A Spawning Time Series")
...
We took the average of the value for `MR3A` (`0.92`) for the Lower McCloud and the value for the station `MRA` (`0.99`) representing the Upper McCloud resulting in a mean proportion of `0.95`. 
...
The value by which to scale the total egg-to-fry survival is `0.95 / 0.59 = 1.61`. 

### Little Upper Sacramento River (ASD-5 & ASD-6)

![](md_graphs/md_asl_map.png)

We used `11342000` as the relevant gage for representing spawning habitat in the Little Upper Sacramento River. The value by which to scale the total egg-to-fry survival is `0.18 / 0.59 = 0.3`.
...
wr_sdm_egg_to_fry_abv_dam_values <- tibble("Station ID" = c("11342000", "MRA", "MR3A", NA_character_),
                          "Watershed" = c("Little Sacramento River", "Upper McCloud River", "Lower McCloud River", "Full McCloud River"),
                          "Egg-to-Fry Scale Factor" = c(0.30, 1.67, 1.56, 1.61))

wr_sdm_egg_to_fry_abv_dam_values |> 
  knitr::kable()
```