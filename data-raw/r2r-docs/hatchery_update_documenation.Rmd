...
1)  Each year, if model is run deterministicly, there are 307650 hatchery adults initiated into the model based on (CWT data?). Hatchery adults are split between tribs based on values from `winterRunDSM::hatchery_allocation`
2)  Percent natural is output from model in the `apply_enroute_survival()` submodel
...
1) The first two years there are 307650 hatchery adults initiated into the model based on CWT data. In subsequent years hatchery adults are carried over from prior generations and not initiated annually. 
2) The adult removal rate for hatcheries is `0` for years where there are no hatchery releases.
3) Proportion natural is calculated as `1 - proportion_hatchery`. `proportion_hatchery` is calculated from CWT data, grouped by tributary and summarized to find the mean percentage of hatchery fish for each tributary.
4) PHOS initiated as `1 - proportion_natural` and adjusted to describe hatchery fish renaturing every 2 generations. Renaturing adjustment assumes that if PHOS is decreasing in two prior years than the population of hatchery for current year should be adjusted by the percent difference of PHOS in current year vs current year minus two. PHOS is recalculated using adjusted number of hatchery fish divided by total population. If there were no hatchery
releases in the five previous years, `proportion_natural` is set to `1` (i.e. `PHOS = 0`).
5) Both natural adults and hatchery adults spawn. Distinct fecundity is applied to natural vs hatchery fish based on fish size. The model does not track fish size, only age, so ages are mapped to size using logic from Roni and Quinn 1995. 
6) Hatchery releases are added into the model logic in tributaries and when applicable as juveniles at chipps island. Hatchery fish are added in within the large juvenile size class. 
7) All surviving juveniles outmigrate and are carried over to the next year. Hatchery and natural adult returns are tracked separately and have different adult return ratios.
...
Each year PHOS is initiated as 1 - percent_natural. PHOS is updated to account for renaturing every 2 generations if PHOS decreases in last two consecutive years. If there were no hatchery
releases in the five previous years, PHOS is set to `0`.
...
We assume the current fecundity used in the DSMs (5222) is fecundity for age 3 wild fish. We scaled up or down using logic from 1 - mm reduction in length results in 7.8 fewer eggs (95% CI = 6.6–8.9). Logic from Malick, M.J., Losee, J.P., Marston, G., Agha, M., Berejikian, B.A., Beckman, B.R. and Cooper, M., 2023. Fecundity trends of Chinook salmon in the Pacific Northwest. Fish and Fisheries.
...
fecundity_table <- tibble(`Age in Model` = c(2, 2, 3, 3, 4, 4, 5),
                          ...
                          Fecundity = c((winterRunDSM::params$spawn_success_fecundity - ((69.1 - 50.9) * 10 * 7.8)), 
                                        (winterRunDSM::params$spawn_success_fecundity - ((69.1 - 48.9) * 10 * 7.8)), 
                                        winterRunDSM::params$spawn_success_fecundity, 
                                        (winterRunDSM::params$spawn_success_fecundity - ((69.1 -  67.1) * 10 * 7.8)), 
                                        (winterRunDSM::params$spawn_success_fecundity - ((69.1 - 83.0) * 10 * 7.8)),
                                        (winterRunDSM::params$spawn_success_fecundity - ((69.1 - 81.0) * 10 * 7.8)),
                                        (winterRunDSM::params$spawn_success_fecundity - ((69.1 - 85) * 10 * 7.8)))) |> 
...
Hatchery return data comes from the production targets defined in the California [HSRG](California HSRG: https://swfsc-publications.fisheries.noaa.gov/publications/CR/2012/2012California.pdf)(Pages 65 - 95).
...
winter_hatchery_release[, 3] <- summarized_release$release_number
